...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1Obj.x2 > rect2Obj.x1 && rect1Obj.y2 > rect2Obj.y1) return true;
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (!acc.includes(str[i]) && str.lastIndexOf(str[i]) === i) {
      return str[i];
    }
    acc.push(str[i]);
  }
...
function getCommonDirectoryPath(pathes) {
  const path1 = pathes[0];
  const path2 = pathes[1];
  const resArr = [];
  ...
  const rest = [res].concat(pathes.slice(2));
  const res2 = res.slice(0, 1 + res.lastIndexOf('/'));
  return (pathes.length < 3) ? res2 : getCommonDirectoryPath(rest);
}
